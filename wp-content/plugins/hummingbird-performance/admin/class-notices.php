...
class Notices {
	 ...
	 */
	private function show_notice( $id = '', $message = '', $additional = false, $only_hb_pages = false, $notice_class = 'info' ) {
		// Only run on HB pages.
		if ( $only_hb_pages && ! preg_match( '/^(toplevel|hummingbird)(-pro)*_page_wphb/', get_current_screen()->id ) ) {
			return;
		}
	...
	public function safe_mode_notice() {
		...
		$message = sprintf(
			...
			esc_html__( '%1$sSafe Mode%2$s is active. Adjust your Hummingbird settings and preview the changes. When you’re ready, publish them live — or disable Safe Mode at any time. Keeping Safe Mode on for too long may slow your live site.', 'wphb' ),
			'<strong>',
			'</strong>'
		);
