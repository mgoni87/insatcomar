...
class New_Feature extends Controller {
	...
	public function update_new_feature_status() {
		if (
			...
			wp_send_json_error( array( 'message' => __( 'No Permission to set.', 'smartcrawl-seo' ) ) );
		}
		...
		if ( ! isset( $data['step'] ) || '-1' === $data['step'] ) {
			wp_send_json_error( array( 'message' => __( 'Feature is not found.', 'smartcrawl-seo' ) ) );
		}

		$curr_step = (int) Settings::get_specific_options( 'wds-features-viewed', 0 );

		if ( $curr_step < (int) $data['step'] ) {
			Settings::update_specific_options( 'wds-features-viewed', $data['step'] );
		}

		wp_send_json_success( array( 'message' => __( 'New feature status was successfully updated.', 'smartcrawl-seo' ) ) );
	}
	...
	public function update_new_feature_badge() {
		if ( ! isset( $_POST['_wds_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wds_nonce'] ) ), 'wds-admin-nonce' ) ) {
			wp_send_json_error( array( 'message' => __( 'Nonce verification failed.', 'smartcrawl-seo' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'New feature badge was successfully updated.', 'smartcrawl-seo' ) ) );
	}
