...
class Controller extends Controllers\Submodule_Controller {
	...
	public function display_upgrade_notice() {
		...
		$redirects_admin_url  = Admin_Settings::admin_url( Settings::ADVANCED_MODULE ) . '&tab=' . $this->module_name;
		...
		>
			...
			>
				<?php esc_html_e( 'Go to Redirects', 'smartcrawl-seo' ); ?>
			</a>
			<a href="#" class="wds-native-dismiss"><?php esc_html_e( 'Dismiss', 'smartcrawl-seo' ); ?></a>
			<p></p>
		</div>
		<?php
	}
			...
			<p>
				...
				echo wp_kses_post(
					...
					/* translators: 1, 2: strong tag, 3: post type label, 4: post stats transition action, 5, 6: Link to redirection page, 7: Redirection source url. */
						esc_html__( '%1$sSEO Notice:%2$s A published %3$s(%8$s%7$s%9$s) has been moved to %4$s. %5$sAdd redirection%6$s to improve SEO.', 'smartcrawl-seo' ),
						'<strong>',
						'</strong>',
						...
						/* translators: 1: Advanced Tools page url, 2: Redirection source url */
							'<a href="%1$s&tab=%2$s&add_redirect=1&source=%3$s">',
	...
	public function render_dashboard_content() {
		...
		$module_url = Admin_Settings::admin_url( $this->parent->module_name ) . '&tab=' . $this->module_name . '&add_redirect=1';
		?>
	...
	public function sanitize_options( $input ) {
		...
		$this->options['attachments']  = (bool) $input['attachments'];
		$this->options['images_only']  = (bool) $input['images_only'];
		...
		if ( empty( $destination['id'] ) ) {
			...
			if ( $post_url ) {
				...
				if ( $post_type_obj ) {
					...
					$destination['type'] = __( 'Url', 'smartcrawl-seo' );
				}
			}
		}
	...
	public function delete_redirect() {
		...
		if ( empty( $data ) ) {
			wp_send_json_error( array( 'mesage' => __( 'Request data is not valid.', 'smartcrawl-seo' ) ) );
		}
		...
		wp_send_json_error( array( 'message' => __( 'Something was wrong.', 'smartcrawl-seo' ) ) );
	}
	...
	public function import_redirects() {
		...
		if ( $file_size > 1000000 ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Please select a file under 1MB.', 'smartcrawl-seo' ),
				)
			);
		}
		...
		if ( in_array( $file_type, \smartcrawl_csv_mime_types(), true ) ) {
			...
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Only JSON or CSV files are supported.', 'smartcrawl-seo' ),
				)
			);
		}
		...
		if ( $errors ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Some entries have invalid values. Please try again!', 'smartcrawl-seo' ),
				)
			);
		}
		...
		if ( empty( $redirects ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'No valid redirects were found, please check your file.', 'smartcrawl-seo' ),
				)
			);
		}
		...
		if ( ! $inserted ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'An error occurred while inserting CSV data into the database.', 'smartcrawl-seo' ),
				)
			);
		}
	...
	public function export_redirects() {
		...
		if ( ! $redirects ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Please save some redirects first.', 'smartcrawl-seo' ),
				)
			);
		}
