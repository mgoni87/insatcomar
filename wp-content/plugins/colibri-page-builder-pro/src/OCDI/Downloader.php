...
class Downloader {
    ...
    private function get_content_from_url( $url ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error(
                'missing_url',
                __( 'Missing URL for downloading a file!', 'colibri-page-builder' )
            );
        }
        ...
        $response = wp_remote_get(
            $url,
            array(
                'timeout' => apply_filters( 'extendthemes-ocdi/timeout_for_downloading_import_file', 30 ) ,
                'sslverify'  => false,
            )
        );
        ...
        if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
            ...
            return new WP_Error(
                ...
                apply_filters( 'extendthemes-ocdi/message_after_file_fetching_error', '' )
            );
        }
    ...
    public function set_download_directory_path( $download_directory_path ) {
        if ( file_exists( $download_directory_path ) ) {
            ...
            $this->download_directory_path = apply_filters( 'extendthemes-ocdi/upload_file_path', trailingslashit( $upload_dir['path'] ) );
        }
    }
}
