...
class WPCLICommands extends WP_CLI_Command {
    ...
    public function import( $args, $assoc_args ) {
        if ( ! $this->any_import_options_set( $assoc_args ) ) {
            WP_CLI::error( esc_html__( 'At least one of the possible options should be set! Check them with --help', 'colibri-page-builder') );
        }
    ...
    private function import_predefined( $predefined_index ) {
        if ( ! is_numeric( $predefined_index ) ) {
            WP_CLI::error( esc_html__( 'The "predefined" parameter should be a number (an index of the OCDI predefined demo import)!', 'colibri-page-builder') );
        }

        $predefined_index = absint( $predefined_index );

        if ( ! array_key_exists( $predefined_index, $this->ocdi->import_files ) ) {
            WP_CLI::warning( esc_html__( 'The supplied predefined index does not exist! Please take a look at the available predefined demo imports:', 'colibri-page-builder') );
        ...
        WP_CLI::log( esc_html__( 'Predefined demo import started! All other parameters will be ignored!', 'colibri-page-builder') );
        ...
        WP_CLI::log( esc_html__( 'Preparing the demo import files...', 'colibri-page-builder') );

        $import_files = Helpers::download_import_files( $selected_files );

        if ( empty( $import_files ) ) {
            WP_CLI::error( esc_html__( 'Demo import files could not be retrieved!', 'colibri-page-builder') );
        }

        WP_CLI::log( esc_html__( 'Demo import files retrieved successfully!', 'colibri-page-builder') );

        WP_CLI::log( esc_html__( 'Importing...', 'colibri-page-builder') );

        if ( ! empty( $import_files['content'] ) ) {
            $this->do_action( 'extendthemes-ocdi/before_content_import_execution', $import_files, $this->ocdi->import_files, $predefined_index );

            $this->import_content( $import_files['content'] );
        }

        if ( ! empty( $import_files['widgets'] ) ) {
            $this->do_action( 'extendthemes-ocdi/before_widgets_import', $import_files );
        ...
        $this->do_action( 'extendthemes-ocdi/after_all_import_execution', $import_files, $this->ocdi->import_files, $predefined_index );

        WP_CLI::log( esc_html__( 'Predefined import finished!', 'colibri-page-builder') );
    }
    ...
    public function list_predefined() {
        if ( empty( $this->ocdi->import_files ) ) {
            WP_CLI::error( esc_html__( 'There are no predefined demo imports for currently active theme!', 'colibri-page-builder') );
        }

        WP_CLI::success( esc_html__( 'Here are the predefined demo imports:', 'colibri-page-builder') );
    ...
    private function import_content( $relative_file_path ) {
        ...
        if ( empty( $this->ocdi->frontend_error_messages ) ) {
            ...
            WP_CLI::warning( esc_html__( 'There were some issues while importing the content!', 'colibri-page-builder') );
    ...
    private function import_widgets( $relative_file_path ) {
        ...
        if ( ! file_exists( $widgets_import_file_path ) ) {
            WP_CLI::warning( esc_html__( 'Widgets import file provided does not exist! Skipping this import!', 'colibri-page-builder') );

            return false;
        }

        WP_CLI::log( esc_html__( 'Importing widgets...', 'colibri-page-builder') );

        WidgetImporter::import( $widgets_import_file_path );

        if ( empty( $this->ocdi->frontend_error_messages ) ) {
            WP_CLI::success( esc_html__( 'Widgets imported successfully!', 'colibri-page-builder') );
        } else {
            WP_CLI::warning( esc_html__( 'There were some issues while importing widgets!', 'colibri-page-builder') );
    ...
    private function import_customizer( $relative_file_path ) {
        ...
        if ( ! file_exists( $customizer_import_file_path ) ) {
            WP_CLI::warning( esc_html__( 'Customizer import file provided does not exist! Skipping this import!', 'colibri-page-builder') );

            return false;
        }

        WP_CLI::log( esc_html__( 'Importing customizer settings...', 'colibri-page-builder') );

        CustomizerImporter::import( $customizer_import_file_path );

        if ( empty( $this->ocdi->frontend_error_messages ) ) {
            WP_CLI::success( esc_html__( 'Customizer settings imported successfully!', 'colibri-page-builder') );
        } else {
            WP_CLI::warning( esc_html__( 'There were some issues while importing customizer settings!', 'colibri-page-builder') );
