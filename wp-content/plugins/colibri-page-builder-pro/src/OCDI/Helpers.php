...
class Helpers {
    ...
    public static function download_import_files( $import_file_info ) {
        ...
        $import_file_info = apply_filters( 'extendthemes-ocdi/pre_download_import_files', $import_file_info );
        ...
        if ( empty( $import_file_info['import_file_url'] ) ) {
            ...
            $content_filename = apply_filters( 'extendthemes-ocdi/downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . apply_filters( 'extendthemes-ocdi/downloaded_content_file_suffix_and_file_extension', '.xml' );
        ...
        if ( ! empty( $import_file_info['import_widget_file_url'] ) ) {
            ...
            $widget_filename = apply_filters( 'extendthemes-ocdi/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . apply_filters( 'extendthemes-ocdi/downloaded_widgets_file_suffix_and_file_extension', '.json' );
        ...
        if ( ! empty( $import_file_info['import_customizer_file_url'] ) ) {
            ...
            $customizer_filename = apply_filters( 'extendthemes-ocdi/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . apply_filters( 'extendthemes-ocdi/downloaded_customizer_file_suffix_and_file_extension', '.dat' );
        ...
        if ( ! empty( $import_file_info['import_redux'] ) && is_array( $import_file_info['import_redux'] ) ) {
            ...
            foreach ( $import_file_info['import_redux'] as $index => $redux_item ) {
                $redux_filename = apply_filters( 'extendthemes-ocdi/downloaded_redux_file_prefix', 'demo-redux-import-file_' ) . $index . '-' . self::$demo_import_start_time . apply_filters( 'extendthemes-ocdi/downloaded_redux_file_suffix_and_file_extension', '.json' );
    ...
    private static function check_wp_filesystem_credentials() {
        ...
        $plugin_page_setup = apply_filters( 'extendthemes-ocdi/plugin_page_setup', array(
                'parent_slug' => 'themes.php',
                'page_title'  => esc_html__( 'One Click Demo Import', 'colibri-page-builder'),
                'menu_title'  => esc_html__( 'Import Demo Data', 'colibri-page-builder'),
                'capability'  => 'import',
                'menu_slug'   => 'pt-one-click-demo-import',
            )
        );

	    $parent_slug = isset($plugin_page_setup['parent_slug']) ? $plugin_page_setup['parent_slug'] : null;
	    $menu_slug = isset($plugin_page_setup['menu_slug']) ? $plugin_page_setup['menu_slug'] : null;
	    // Get user credentials for WP file-system API.
	    if($parent_slug && $menu_slug) {
		    $demo_import_page_url = wp_nonce_url( $parent_slug . '?page=' .$menu_slug, $menu_slug );
	    } else {
        ...
        if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
            return new WP_error(
                ...
                __( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'colibri-page-builder')
            );
        }
        ...
        if ( ! WP_Filesystem( $creds ) ) {
            return new WP_Error(
                ...
                __( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'colibri-page-builder')
            );
        }
    ...
    public static function get_log_path() {
        ...
        $upload_path = apply_filters( 'extendthemes-ocdi/upload_file_path', trailingslashit( $upload_dir['path'] ) );

        $log_path = $upload_path . apply_filters( 'extendthemes-ocdi/log_file_prefix', 'log_file_' ) . self::$demo_import_start_time . apply_filters( 'extendthemes-ocdi/log_file_suffix_and_file_extension', '.txt' );
    ...
    public static function register_file_as_media_attachment( $log_path ) {
        ...
        $filetype  = wp_check_filetype( basename( $log_path ), apply_filters( 'extendthemes-ocdi/file_mimes', $log_mimes ) );

        // Prepare an array of post data for the attachment.
        $attachment = array(
            'guid'           => self::get_log_url( $log_path ),
            'post_mime_type' => $filetype['type'],
            'post_title'     => apply_filters( 'extendthemes-ocdi/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'colibri-page-builder') ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
            'post_content'   => '',
            'post_status'    => 'inherit',
        );
    ...
    public static function get_log_url( $log_path ) {
        $upload_dir = wp_upload_dir();
        $upload_url = apply_filters( 'extendthemes-ocdi/upload_file_url', trailingslashit( $upload_dir['url'] ) );
    ...
    public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
        ...
        if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
            ...
            $log_added = self::append_to_file(
                ...
                $log_file_path,
                esc_html__( 'Upload files', 'colibri-page-builder')
            );
        }
        ...
        if ( $redux_file_info && ! isset( $redux_file_info['error'] ) ) {
            ...
            if ( isset( $_POST['redux_option_name'] ) && empty( $_POST['redux_option_name'] ) ) {
                ...
                self::log_error_and_send_ajax_response(
                    esc_html__( 'Missing Redux option name! Please also enter the Redux option name!', 'colibri-page-builder'),
                    $log_file_path,
                    esc_html__( 'Upload files', 'colibri-page-builder')
                );
            }
        ...
        $log_added = self::append_to_file(
            __( 'The import files were successfully uploaded!', 'colibri-page-builder') . self::import_file_info( $selected_import_files ),
            $log_file_path,
            esc_html__( 'Upload files', 'colibri-page-builder')
        );
    ...
    public static function import_file_info( $selected_import_files ) {
        ...
        return PHP_EOL .
               ...
               sprintf(
                   ...
                   PHP_EOL,
                   get_site_url(),
                   empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'colibri-page-builder') : $selected_import_files['content'],
                   empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'colibri-page-builder') : $selected_import_files['widgets'],
                   empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'colibri-page-builder') : $selected_import_files['customizer'],
                   empty( $redux_file_string ) ? esc_html__( 'not defined!', 'colibri-page-builder') : $redux_file_string
               );
    }
